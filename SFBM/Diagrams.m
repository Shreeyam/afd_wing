...
xlabel(xlabeltext,'FontSize',12, 'interpreter','latex'); 
...
%% Creating Shear Force and Bending Moment Diagram
...
%% Shear Force
subplot(2,1,1);
fill([x;flipud(x)],[zeros(size(x));flipud(sf)],'b','facealpha',0.15,'edgecolor','none'); 
hold on; plot(x,sf,'b','Linewidth',2);
offset = 0.01 + 0.05*(max(sf) - min(sf));
Vmin = min(sf) - offset; Vmax = max(sf) + offset;
axis([Xmin, Xmax, Vmin, Vmax]);
plot([Xmin, Xmax], [0,0], 'k'); hold off;
H = (Vmax - Vmin)/20; diffY = [diff(YtickSF)]; YtickSF = YtickSF(diffY > H);
W = (Xmax - Xmin)/20; diffX = [diff(Xtick)]; Xtick2 = Xtick(diffX > W);
ax3 = gca; ax3.YTick = YtickSF; ax3.XTick = Xtick2;
ax3.FontSize = 12; ax3.TickLabelInterpreter = 'latex'; 
if SFfactor > 1
...
xlabel(xlabeltext,'FontSize',12, 'interpreter','latex'); 
ylabel(ylabeltext,'FontSize',12, 'interpreter','latex');  
title('$Shear~Force~Diagram$','FontSize',12, 'interpreter','latex')

%% Bending Moment
subplot(2,1,2);
fill([x;flipud(x)],[zeros(size(x));flipud(bm)],'b','facealpha',0.15,'edgecolor','none'); 
hold on; plot(x,bm,'b','Linewidth',2);
offset = 0.01 + 0.05*(max(bm) - min(bm));
Mmin = min(bm) - offset; Mmax = max(bm) + offset;
axis([Xmin, Xmax, Mmin, Mmax]);
plot([Xmin, Xmax], [0,0], 'k'); hold off;
H = (Mmax - Mmin)/20; diffY = [diff(YtickBM)]; YtickBM = YtickBM(diffY > H);
ax4 = gca; ax4.YTick = YtickBM; ax4.XTick = Xtick2;
ax4.FontSize = 12; ax4.TickLabelInterpreter = 'latex';
if BMfactor > 1
